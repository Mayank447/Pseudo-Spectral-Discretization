...
class DiracOperator:
    def __init__(self, N: int):
        self.N = N
        self.dx = L / N
        # self.A = 1j * (np.kron(self.Dy, np.eye(N)) + B * np.kron(np.eye(N), self.Dy))
    ...
    def dirac_operator(self, L, B, gamma1, gamma2):
        """
        ...
        A = np.kron(gamma1, D_x) + np.kron(gamma2, D_y) + np.kron(gamma0, B * np.kron(I_N, I_N))
    ...
    def construct_dirac_eigenmodes(self, eigenvalues, eigenvectors):
        """
        Constructs the eigenmodes of the original Dirac operator from the eigenmodes of the squared operator.
        """
        dirac_modes = []
...
def main():
    ...
    dirac_op = D.dirac_operator(L, B, gamma1, gamma2)
