...
def test_exposes_x(num_points, L):
    assert np.allclose(FreeFermion2D(*num_points, *L).x, np.meshgrid(np.linspace(0, L[0], num_points[0], endpoint=False), np.linspace(0, L[1], num_points[1], endpoint=False)))
